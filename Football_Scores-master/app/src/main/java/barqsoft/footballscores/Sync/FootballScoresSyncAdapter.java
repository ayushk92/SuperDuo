...
public class FootballScoresSyncAdapter extends AbstractThreadedSyncAdapter {
    ...
    private void getData (String timeFrame)
    {
        ...
        try {
            ...
            m_connection.addRequestProperty("X-Auth-Token",getContext().getResources().getString(R.string.api_key));
        ...
        try {
            if (JSON_data != null) {
                Log.d(LOG_TAG,"Loading dummy data");
                //This bit is to check if the data contains any matches. If not, we call processJson on the dummy data
                JSONArray matches = new JSONObject(JSON_data).getJSONArray("fixtures");
                if (matches.length() == 0) {
                    //if there is no data, call the function on dummy data
                    //this is expected behavior during the off season.
                    processJSONdata(getContext().getResources().getString(R.string.dummy_data), getContext(), false);
                    return;
                }
